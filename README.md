...
3. The system sends an email notification asynchronously — the API does not block waiting for the email.
...
- Security: cors, helmet, rate-limit, sensible
...
- Messaging: Producer/Consumer, Transactional Outbox, Idempotent Consumer, Dead Letter Queue, Retry + exponential backoff
- Reliability & ops: Graceful Shutdown, Health/Readiness, Structured Logging + Correlation ID, 12-Factor config
